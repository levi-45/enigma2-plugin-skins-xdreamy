...
def Plugins(**kwargs):
    return [PluginDescriptor(
            ...
            fnc=autostart)]
    # return PluginDescriptor(name='XDREAMY', description=_('Customization tool for XDREAMY Skin'), where=PluginDescriptor.WHERE_PLUGINMENU, icon='plugin.png', fnc=main)
...
class xDreamySetup(ConfigListScreen, Screen):
    ...
    def mesInfo(self):
        message = (
            ...
            "Supported Images\n"
            "Egami 10.x, OpenATV 6.x, OpenATV 7.x\n"
            "OpenSpa 8.x, PurE2 6.x, PurE2 7.x, OpenDroid & Alliance Based Images\n"
            "(OpenPLi, OpenVIX)\n"
            "(OpenBH)\n"
    ...
    def createSetup(self):
        try:
            ...
            list.append(getConfigListEntry(section))
            list.append(getConfigListEntry(_('Skin Style:'), config.plugins.xDreamy.skinSelector))
            ...
            list.append(getConfigListEntry(_('Channel Name Color:'), config.plugins.xDreamy.channelnamecolor))
            ...
            list.append(getConfigListEntry(_('ChannelSelection Style:'), config.plugins.xDreamy.ChannSelector))
            list.append(getConfigListEntry(_('EventView Style:'), config.plugins.xDreamy.EventView))
            list.append(getConfigListEntry(_('VolumeBar Style:'), config.plugins.xDreamy.VolumeBar))
            list.append(getConfigListEntry(_('PluginBrowser Style:'), config.plugins.xDreamy.PluginBrowser))
            list.append(getConfigListEntry(_('VirtualKeyboard Style:'), config.plugins.xDreamy.VirtualKeyboard))
            list.append(getConfigListEntry(_('HistoryZapSelector Style:'), config.plugins.xDreamy.HistoryZapSelector))
            section = '\\c00289496' + _('---------------------------( USER PLUGINS SCREENS )------------------------')
            list.append(getConfigListEntry(section))
            list.append(getConfigListEntry(_('NewVirtualKeyboard Style:'), config.plugins.xDreamy.NewVirtualKeyboard))
            list.append(getConfigListEntry(_('EPGMultiSelection Style:'), config.plugins.xDreamy.EPGMultiSelection))
            list.append(getConfigListEntry(_('E2Player Style:'), config.plugins.xDreamy.E2Player))
            list.append(getConfigListEntry(_('EnhancedMovieCenter Style:'), config.plugins.xDreamy.EnhancedMovieCenter))
            section = '\\c00289496' + _('-------------------------------( USER BACKGROUND )-------------------------')
            list.append(getConfigListEntry(section))
            list.append(getConfigListEntry(_('ChannelListBackground Style:'), config.plugins.xDreamy.ChannelListBackground))
    ...
    def GetPicturePath(self):
        ...
        # OAWeatherpng = '/usr/share/enigma2/xDreamy/screens/OAWeatherPlugin.png'
        # MsnWeatherPng = '/usr/share/enigma2/xDreamy/screens/MSNWeatherPlugin.png'
    ...
    def checkforUpdate(self):
        try:
            ...
            req = Request('https://raw.githubusercontent.com/Insprion80/Skins/main/xDreamy/xD.txt')
